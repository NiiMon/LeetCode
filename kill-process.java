...
class Solution {
    public List<Integer> killProcess(List<Integer> pid, List<Integer> ppid, 
        int kill) {
        Map<Integer, List<Integer>> graph = deserialze(pid, ppid);
        return walk(kill, graph);
    }
    private List<Integer> walk(int kill, Map<Integer, List<Integer>> graph) {
        ...
        if (graph.containsKey(kill)) {
            for (int next : graph.get(kill)) {
                result.addAll(walk(next, graph));
            }
        }
        
        return result;
    }
    private Map<Integer, List<Integer>> deserialze(List<Integer> pid, 
        List<Integer> ppid) {
